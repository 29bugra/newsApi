...
class HomeViewModel : ViewModel() {
    ...
    fun runTwo(country: String) {

        getDataApıı(country)
    }

    fun runThree(currentType:String,countryText:String,hoodText:String,language:String,from:String,date:String,sortBy:String) {
    ...
    public fun getDataApııı(currentType:String,countryText:String,hoodText:String,language:String,from:String,date:String,sortBy:String) {
        disposable.add(

            newsApiService.getNewsApi()
                .getSearch("$currentType","$countryText","$hoodText","$language","$from","$date","$sortBy","632731ff030d44a3885c56f99b626125")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                    {
